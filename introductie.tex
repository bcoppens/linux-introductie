...
De studenten die om 10u al aan de Ufo moeten staan kunnen gerust wat vroeger vertrekken.
...
\begin{enumerate}
  ...
  \item Met enkele oefeningen op Dodona. Deze oefeningen zullen gradueel
    ...
    \url{https://dodona.be/nl/courses/3376/}. De ochtend van elk van de PC-sessies,
...
Om hier wat mee te oefenen, \emph{had} de UGent vroeger een ssh-server draaien waar
studenten mee kunnen connecteren. Helaas hebben ze die afgevoerd :-( Nu, ze bieden (in hun woorden)
een `volwaardig alternatief' aan, in de vorm van de HPC (High Performance Computing) infrastructuur. Dat is
inderdaad een van de vele toepassingen voor een remote shell (net als het beheer van `gewone' serverinfrastructuur).
Voor beginnende student is dat misschien wel een beetje overkill, maar bekijk het zo: dan heb je meteen
al een account en ervaring ermee voor later :-)


% Deze server heet \emph{helios}, en is enkel
...
De volledige documentatie ga ik hier niet inplakken, die kan je vinden op \url{https://www.ugent.be/hpc/en/support/documentation.htm}. In het bijzonder zal je de \url{http://hpcugent.github.io/vsc_user_docs/pdf/intro-HPC-linux-gent.pdf} Linux introductie / howto moeten gebruiken. De hoofdstukken die voor jullie het belangrijste zijn, zijn Hoofdstuk 2 over `Getting an HPC Account', en Hoofdstuk 3 over `Connecting to the HPC infrastructure'.

Om dan te connecteren naar die server, gebruik je \verb!ssh !\emph{HPC-gebruikersnaam}\verb!@login.hpc.ugent.be!. In mijn geval ziet dit er als volgt uit:
\begin{verbatim}
...
Warning: Permanently added 'login.hpc.ugent.be' (ECDSA) to the list of known hosts.

 STEVIN HPC-UGent infrastructure status on Fri, 03 Mar 2023 16:45:02   
      cluster     - full - free -  part - total - running - queued     
                    nodes  nodes   free   nodes   jobs      jobs       
 --------------------------------------------------------------------- 
      slaking           0      1      5       7         6        2     
       swalot         101      0     14     117      2195    13869     
       skitty           5      0     53      71       369     4330     
      victini          10      0     85      96       728    26958     
       joltik           6      0      2      10        25        2     
       kirlia           6      0      2       8         2     6018     
        doduo          61      0     49     128      1940    30367     
     accelgor           9      0      0       9        30       47     
      donphan           0      7      1       8         1        0     
...
Last login: Fri Mar  3 16:38:01 2023 from X.X.X.X
vscXXXXX@gligar08 ~ $
\end{verbatim}
...
\item Mijn gebruikersnaam op de HPC is anders dan die op mijn lokale machine, dus die geef ik expliciet mee.

\item Er wordt ogenschijnlijk \emph{niet} gevraagd naar je wachtwoord!
    Dit komt omdat er gebruik gemaakt wordt van identificatie aan de hand van
    een private sleutel. Als je ook een HPC-account aanmaakt, is het dan ook
    \textbf{belangrijk om (het bestand met) die private sleutel niet aan anderen
    te geven!} Dat is alsof je iemand anders je UGent-wachtwoord zou geven!

\item Achter de schermen blijk ik ingelogd te zijn op de \verb!gligar08!-server.
\end{itemize}

Als je dit zou proberen, kan je als voorbeeld bijvoorbeeld eens kijken
naar hoe groot de schijfruimte is beschikbaar is op de HPC.

Een aantal commando's die wellicht eens leuk zijn om mee te experimenteren
zijn: \verb!ps! om een tekstueel overzicht te geven van alle draaiende
...
worden. Probeer eens al deze commando's uit op de HPC.

Merk daarbij op dat, in tegenstelling tot wat er gebeurt op je lokale Linux-machine,
je \emph{zeer} weinig processen zal zien. De HPC-infrastructuur is immers vrij afgeschermd:
je kan niet zien wat andere gebruikers aan het doen zijn. (Het idee is dat mensen
zouden kunnen proberen afleiden welk onderzoek je aan het doen bent, gebaseerd op welke
programma's met welke argumenten je aan het draaien bent.)
...
Nu, als je dat later in een niet-HPC context zou willen gebruiken, zijn er nog
een aantal extra tips / aandachtspunten:
\begin{itemize}
\item Als je op een systeem wil inloggen waar je wachtwoord toch nodig is,
   zal er bij het intypen van je wachtwoord \emph{niets naar het scherm
  geprint worden!} Dus in tegenstelling tot wat je misschien verwacht, zal je géén
...
\item De prompt die je krijgt, kan er soms anders uitzien.  Soms
    wordt er immers een andere shell gebruikt wordt dan degene die we totnogtoe gebruikt
    ...
    Andere shells zijn bijvoorbeeld de \verb!zsh!-shell, de \verb!ksh!-shell, etc. Wil je in de
...
\item Het gebruik van ssh kan trouwens ook handig zijn als je thuis bijvoorbeeld een
